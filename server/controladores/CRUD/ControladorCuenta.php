...
class ControladorCuenta extends ControladorBase
{
   ...
   {
      $sql = "INSERT INTO Cuenta (nickname,idPersona,idRol,clave) VALUES (?,?,?,AES_ENCRYPT(?,?));";
      $parametros = array($cuenta->nickname,$cuenta->idPersona,$cuenta->idRol,$cuenta->clave,'CLAVE DE CIFRADO');
   ...
   {
      $parametros = array($cuenta->nickname,$cuenta->idPersona,$cuenta->idRol,$cuenta->clave,$cuenta->id);
      $sql = "UPDATE Cuenta SET nickname = ?,idPersona = ?,idRol = ?,clave = ? WHERE id = ?;";
      $respuesta = $this->conexion->ejecutarConsulta($sql,$parametros);
      if(is_null($respuesta[0])){
         return true;
      }else{
